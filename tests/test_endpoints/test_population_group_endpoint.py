...
async def test_get_population_group(event_loop, refresh_db):
    ...
    assert len(response.json()) > 0, 'There should be at least one population group in the database'
...
async def test_get_population_group_params(event_loop, refresh_db):
    ...
    for param_name, param_value in query_parameters.items():
        ...
        assert response.status_code == 200
        assert len(response.json()) > 0, f'There should be at least one population group entry for parameter "{param_name}" with value "{param_value}" in the database'

    async with AsyncClient(app=app, base_url='http://test', params=query_parameters) as ac:
        response = await ac.get(ENDPOINT_ROUTER)

    assert response.status_code == 200
    assert len(response.json()) > 0, f'There should be at least one population group for all parameters in the database'
