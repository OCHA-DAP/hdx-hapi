...
ENDPOINT_ROUTER = '/api/v2/metadata/sector'
...
@pytest.mark.asyncio
async def test_get_sector(event_loop, refresh_db):
    log.info('started test_get_sector')
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
        response = await ac.get(ENDPOINT_ROUTER)
    assert response.status_code == 200
    assert len(response.json()['data']) > 0, 'There should be at least one sector in the database'
...
async def test_get_sector_params(event_loop, refresh_db):
    ...
    for param_name, param_value in query_parameters.items():
        ...
        assert len(response.json()['data']) > 0, (
            f'There should be at least one sector entry for parameter "{param_name}" with value "{param_value}" '
            'in the database'
        )
    ...
    assert response.status_code == 200
    assert len(response.json()['data']) > 0, (
        'There should be at least one sector entry for all parameters in the database'
    )


@pytest.mark.asyncio
async def test_get_sector_result(event_loop, refresh_db):
    log.info('started test_get_sector_result')
