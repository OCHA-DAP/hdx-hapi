...
ENDPOINT_ROUTER = '/api/v2/metadata/location'
...
@pytest.mark.asyncio
async def test_get_location(event_loop, refresh_db):
    log.info('started test_get_location')
    async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
        response = await ac.get(ENDPOINT_ROUTER)
    assert response.status_code == 200
    assert len(response.json()['data']) > 0, 'There should be at least one location in the database'
...
async def test_get_location_params(event_loop, refresh_db):
    ...
    for param_name, param_value in query_parameters.items():
        ...
        assert len(response.json()['data']) > 0, (
            'There should be at least one location entry for parameter '
    ...
    assert response.status_code == 200
    assert len(response.json()['data']) > 0, (
        'There should be at least one location entry for all parameters in the database'
    )


@pytest.mark.asyncio
async def test_get_location_result(event_loop, refresh_db):
    log.info('started test_get_location_result')
