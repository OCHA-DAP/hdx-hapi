...
async def test_get_admin2_params(event_loop, refresh_db):
    ...
    for param_name, param_value in query_parameters.items():
        ...
        assert len(response.json()) > 0, \
            'There should be at least one admin2 entry for parameter ' \
    ...
    assert response.status_code == 200
    assert len(response.json()) > 0, 'There should be at least one admin2 entry for all parameters in the database'
