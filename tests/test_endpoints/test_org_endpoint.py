...
ENDPOINT_ROUTER = '/api/v2/metadata/org'
...
async def test_get_orgs(event_loop, refresh_db):
    ...
    assert len(response.json()['data']) > 0, 'There should be at least one org in the database'
...
async def test_get_org_params(event_loop, refresh_db):
    ...
    for param_name, param_value in query_parameters.items():
        ...
        assert len(response.json()['data']) > 0, (
            f'There should be at least one org entry for parameter "{param_name}" with value "{param_value}" '
            'in the database'
        )
    ...
    assert response.status_code == 200
    assert len(response.json()['data']) > 0, 'There should be at least one org entry for all parameters in the database'


@pytest.mark.asyncio
async def test_get_org_result(event_loop, refresh_db):
    log.info('started test_get_org_result')
