...
async def test_get_humanitarian_needs(event_loop, refresh_db):
    ...
    assert len(response.json()) > 0, 'There should be at least one food security entry in the database'
...
async def test_get_humanitarian_needs_params(event_loop, refresh_db):
    ...
    for param_name, param_value in query_parameters.items():
        ...
        assert response.status_code == 200
        assert len(response.json()) > 0, f'There should be at least one humanitarian_needs entry for parameter "{param_name}" with value "{param_value}" in the database'

    async with AsyncClient(app=app, base_url='http://test', params=query_parameters) as ac:
        response = await ac.get(ENDPOINT_ROUTER)

    assert response.status_code == 200
    assert len(response.json()) > 0, f'There should be at least one humanitarian_needs entry for all parameters in the database'
...
async def test_get_humanitarian_needs_adm_fields(event_loop, refresh_db):
    ...
    assert humanitarian_needs_view_adm_specified.admin1_code == 'FOO-XXX', 'admin1_code should keep its value when admin1_is_unspecified is False'
    assert humanitarian_needs_view_adm_specified.admin1_name == 'Province 01', 'admin1_name should keep its value when admin1_is_unspecified is False'
    assert humanitarian_needs_view_adm_specified.admin2_code == 'FOO-XXX-XXX', 'admin2_code should keep its value when admin1_is_unspecified is False'
    assert humanitarian_needs_view_adm_specified.admin2_name == 'District A', 'admin2_name should keep its value when admin1_is_unspecified is False'
    ...
    assert humanitarian_needs_view_adm_unspecified.admin1_code == None, 'admin1_code should be changed to None when admin1_is_unspecified is True'
    assert humanitarian_needs_view_adm_unspecified.admin1_name == None, 'admin1_name should be changed to None when admin1_is_unspecified is True'
    assert humanitarian_needs_view_adm_unspecified.admin2_code == None, 'admin2_code should be changed to None when admin1_is_unspecified is True'
    assert humanitarian_needs_view_adm_unspecified.admin2_name == None, 'admin2_name should be changed to None when admin1_is_unspecified is True'
