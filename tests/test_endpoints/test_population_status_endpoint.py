...
async def test_get_population_status(event_loop, refresh_db):
    ...
    assert len(response.json()) > 0, 'There should be at least one population status in the database'
...
async def test_get_population_status_params(event_loop, refresh_db):
    ...
    for param_name, param_value in query_parameters.items():
        ...
        assert response.status_code == 200
        assert len(response.json()) > 0, f'There should be at least one population status entry for parameter "{param_name}" with value "{param_value}" in the database'

    async with AsyncClient(app=app, base_url='http://test', params=query_parameters) as ac:
        response = await ac.get(ENDPOINT_ROUTER)

    assert response.status_code == 200
    assert len(response.json()) > 0, f'There should be at least one population status for all parameters in the database'
