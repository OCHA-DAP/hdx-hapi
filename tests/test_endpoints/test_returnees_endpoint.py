...
ENDPOINT_ROUTER = '/api/v1/affected-people/returnees'
...
@pytest.mark.asyncio
async def test_get_returnees(event_loop, refresh_db):
    log.info('started test_get_returnees')
    async with AsyncClient(app=app, base_url='http://test') as ac:
        response = await ac.get(ENDPOINT_ROUTER)
    assert response.status_code == 200
    assert len(response.json()['data']) > 0, 'There should be at least one returnees entry in the database'


@pytest.mark.asyncio
async def test_get_returnees_params(event_loop, refresh_db):
    log.info('started test_get_returnees_params')
    ...
    for param_name, param_value in query_parameters.items():
        ...
        assert len(response.json()['data']) > 0, (
            f'There should be at least one returnees entry for parameter "{param_name}" with value "{param_value}" '
            'in the database'
        )
    ...
    ), 'There should be at least one returnees entry for all parameters in the database'


@pytest.mark.asyncio
async def test_get_returnees_result(event_loop, refresh_db):
    log.info('started test_get_returnees_result')
