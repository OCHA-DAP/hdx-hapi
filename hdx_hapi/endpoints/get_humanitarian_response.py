...
@router.get('/api/org', response_model=List[OrgResponse], summary='Get the list of organizations represented in the data available in HAPI')
async def get_orgs(
...
@router.get('/api/org_type', response_model=List[OrgTypeResponse], summary='Get information about how organizations are classified in HAPI')
async def get_org_types(
    pagination_parameters: Annotated[dict, Depends(pagination_parameters)],
    db: AsyncSession = Depends(get_db),
    code: Annotated[str, Query(max_length=32, description=f'{DOC_ORG_TYPE_CODE}', example='433')] = None,
    description: Annotated[str, Query(max_length=512, description=f'{DOC_ORG_TYPE_DESCRIPTION}', example='Donor')] = None,

    output_format: OutputFormat = OutputFormat.JSON,
):
    """There is no agreed standard for the classification of organizations. The codes and descriptions used in HAPI are based on <a href="https://data.humdata.org/dataset/organization-types-beta">this dataset</a>.
    """    
...
@router.get('/api/sector', response_model=List[SectorResponse], summary='Get information about how humanitarian response activities are classified')
async def get_sectors(
    ...
    """There is no consistent standard for the humanitarian sectors. The codes and descriptions used in HAPI are based on <a href="https://data.humdata.org/organization/54255d0b-c6b1-4517-9722-17321f6634ab">this dataset</a>.
    """    
