...
CONFIG = get_config()
DESCRIPTION = """
The Humanitarian API (HDX HAPI) is a service of the <a href="https://data.humdata.org">Humanitarian Data Exchange (HDX)</a>, part of UNOCHA\'s <a href="https://centre.humdata.org">Centre for Humanitarian Data</a>.\nThis is the reference documentation of the API. You may want to <a href="https://hdx-hapi.readthedocs.io/en/latest/">get started here</a>

All queries require an `app_identifier` which can be supplied as a query parameter or as a header (`X-HDX-HAPI-APP-IDENTIFIER`). The `app_identifier` is simply a base64 encoded version of a user supplied application name and email address.

The `limit` and `offset` parameters are available for all queries and have the usual database meanings to provide pagination of results.

The `output_format` parameter is available for all queries and can be set to JSON or csv, where JSON is selected rows of data are supplied under a data key.

Query parameters that access string fields are implicitly wildcards and case insensitive so that `location_name=Mali` will return data for Mali and Somalia.
"""
